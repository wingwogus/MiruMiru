...
class MailService(
    ...
    private val APP_NAME = "MiruMiru"

    override fun sendAuthCode(email: String, code: String) {
        sendEmail(email, "$APP_NAME 이메일 인증 번호", code)
    }
        ...
        try {
            MimeMessageHelper(mimeMessage, true, "UTF-8").apply {
                ...
                setText("""
                    ...
                    <p style='padding-top: 20px; font-size: 18px; opacity: 0.6; line-height: 30px; font-weight: 400;'>안녕하세요? ${APP_NAME}입니다.<br />
                    $APP_NAME 서비스 사용을 위해 회원가입시 고객님께서 입력하신 이메일 주소의 인증이 필요합니다.<br />
                    하단의 인증 번호로 이메일 인증을 완료하시면, 정상적으로 $APP_NAME 서비스를 이용하실 수 있습니다.<br />
                    항상 최선의 노력을 다하는 $APP_NAME 되겠습니다.<br />
                    감사합니다.</p>
            ...
            throw kotlin.IllegalStateException("이메일 양식 생성 중 오류가 발생했습니다.", e)
        }
        return mimeMessage
    }
}
