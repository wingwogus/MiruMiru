...
class ChatModerationService(
    ...
    fun unblock(command: ChatModerationCommand.Unblock): ChatModerationResult.Unblocked {
        ...
        val deleted = chatBlockRepository.deleteByMember1IdAndMember2Id(firstId, secondId)
    ...
    fun getBlocks(query: ChatModerationQuery.GetBlocks): ChatModerationResult.BlockList {
        val blocks = chatBlockRepository.findAllByMember1IdOrMember2IdOrderByCreatedAtDesc(
            member1Id = query.requesterId,
            member2Id = query.requesterId,
        )

        return ChatModerationResult.BlockList(
            blocks = blocks.map { block ->
                val targetMemberId = if (block.member1.id == query.requesterId) block.member2.id else block.member1.id
    ...
    private fun findBlock(memberAId: Long, memberBId: Long): ChatBlock? {
        ...
        return chatBlockRepository.findByMember1IdAndMember2Id(firstId, secondId)
    }
